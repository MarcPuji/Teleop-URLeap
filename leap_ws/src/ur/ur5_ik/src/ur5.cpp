...
//A UR5 configuration, that can be valid or not
struct Solution {
    Solution() : valid(false) {}
    bool valid;
    UR5::Configuration theta;
};
...
//Solves the UR5 inverse kinematics for the 3rd and 4th joints
...
void find_theta34(const Eigen::AffineCompact3d &transform, Solution *solution,
                  const UR5::Configuration &theta_min,
                  const UR5::Configuration &theta_max,
    ...
    if (in_interval(theta3,theta_min[2],theta_max[2])) {
        ...
        }// else theta4 is out of range
    }// else theta3 is out of range
}


//Solves the UR5 inverse kinematics for the 6th joint and the two possible values of the 2nd joint
...
void find_theta62(const Eigen::AffineCompact3d &transform, Solution *solution,
                  const UR5::Configuration &theta_ref,
                  const UR5::Configuration &theta_min,
                  const UR5::Configuration &theta_max,
    ...
    if (in_interval(theta6,theta_min[5],theta_max[5])) {
        ...
        }// else theta2 has no solution
    }// else theta6 is out of range
}


//Solves the UR5 inverse kinematics for the two possible values of the 5th joint
//and calls the function of the next joints to be found
//joint value will be the nearest one next to the reference
void find_theta5(const Eigen::AffineCompact3d &transform, Solution *solution,
    ...
    }// else theta5 has no solution
}


//Solves the UR5 inverse kinematics for the two possible values of the 1st joint
//and calls the function of the next joints to be found
//joint value will be the nearest one next to the reference
void find_theta1(const Eigen::AffineCompact3d &transform, Solution *solution,
    ...
    }// else theta1 has no solution
}


//Solves the UR5 inverse kinematics to place the TCP in the given pose
//Returns all possible valid solutions
bool UR5::solveIK(const Eigen::AffineCompact3d &transform,
                  std::vector<UR5::Configuration> &theta,
                  const UR5::Configuration &theta_ref,
                  const UR5::Configuration &theta_min,
                  const UR5::Configuration &theta_max) {
    Solution solution[8];
